"""
06_visualizations.py — WordCloud · t-SNE · Courbes LSTM/BERT · Comparaison finale
"""
...
# ── 1. WordClouds ─────────────────────────────────────────────
print("\n[1/5] WordClouds positif vs négatif...")
...
# ── 2. Courbes LSTM ───────────────────────────────────────────
print("\n[2/5] Courbes d'apprentissage LSTM...")
...
# ── 3. Courbes BERT ───────────────────────────────────────────
print("\n[3/5] Courbes d'apprentissage BERT...")
hist_b = results["BERT"]["history"]

# Utiliser val_loss comme référence (3 points = 3 époques)
val_loss = hist_b.get("val_loss", [])
val_acc  = hist_b.get("val_accuracy", [])
n_eval   = len(val_loss)

# Pour train_loss : prendre un point par époque (moyenner les steps)
train_loss_all = hist_b.get("loss", [])
if len(train_loss_all) >= n_eval:
    # Découper en n_eval segments et prendre la moyenne de chaque
...
if len(val_acc) == n_eval and n_eval > 0:
    ...
    # Afficher les stats BERT sous forme de texte
    axes[1].axis("off")
    axes[1].text(0.5, 0.5,
                 f"BERT DistilBERT\n\nF1-score = {results['BERT']['f1']:.4f}\n"
                 f"Époques = 3\nlr = 2e-5\nBatch = 32",
...
# ── 4. t-SNE Word2Vec ─────────────────────────────────────────
print("\n[4/5] t-SNE des embeddings Word2Vec...")
...
ax.set_title("Comparaison finale — F1-score (weighted)",
             fontsize=14, fontweight="bold")
...
print("="*60)
print("\nImages dans results/figures/ :")
import os
for f in sorted(os.listdir("results/figures")):
    if f.endswith(".png"):
        print(f"  OK {f}")
print("\nCopier ces images dans report/figures/")
print("puis compiler report/main.tex")